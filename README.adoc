...
The UI ships with an optional landing-page presentation that keeps the standard navigation, toolbar, and footer while introducing a hero-led first impression for content marked with the `:landing-page:` attribute.
This customization is planned in detail in link:antora-dicio.md[antora-dicio.md] and the supporting notes under [.path]_docs/landing-*.md_.

To enable the landing layout for a page:

. Add `:landing-page:` to the AsciiDoc header and optionally supply hero metadata such as `:page-subtitle:`, `:page-summary:`, `:page-cta-label:`, `:page-cta-url:`, `:page-hero-image:`, value proposition cards (`:page-value1-title:` through `:page-value6-summary:`), and closing CTA values (`:page-cta-bottom-title:`, `:page-cta-bottom-url:`, etc.).
. Build or preview the UI (`gulp preview`) to see the hero section, value proposition grid, and closing CTA card rendered above the article content.
. Bundle the UI and point your Antora playbook at the updated `build/ui-bundle.zip` to deliver the landing experience in production.

The implementation reuses Antora’s existing helpers and partials (`header`, `nav`, `toolbar`, `footer`) while introducing landing-specific partials (`landing-body`, `landing-hero`, `landing-value-prop`, `landing-cta-card`) and a scoped stylesheet imported via `@import "landing.css";`.
Preview the new layout locally via `http://localhost:5252/landing.html` once `gulp preview` is running.
